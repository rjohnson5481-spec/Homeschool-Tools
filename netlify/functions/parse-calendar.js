// Netlify Function: parse-calendar
...
async function checkRateLimit(db, uid) {
  const ref = db.doc(`users/${uid}/aiUsage/calendar`);
...
Each object must have: label (string), startDate (YYYY-MM-DD), endDate (YYYY-MM-DD). If a break is a single day, startDate and endDate are the same. Examples of breaks to extract: Christmas break, Spring break, Fall break, Thanksgiving, teacher inservice days, holidays. Do not include regular school days. Return [] if no breaks are found.`;
...
exports.handler = async function (event) {
  ...
  try {
    ...
    if (!allowed) {
      return {
        statusCode: 429,
        body: JSON.stringify({ error: 'Daily limit reached. You can import up to 5 calendars per day.' }),
      };
    }
  } catch (err) {
  ...
  try {
    ...
    if (isImage) {
      ...
      contentBlock = { type: 'text', text: Buffer.from(file, 'base64').toString('utf-8') };
    }
    ...
    const response = await client.messages.create({
      ...
      messages: [{
        ...
        content: [
          contentBlock,
          { type: 'text', text: `Extract all school breaks and holidays from this calendar. File: ${fileName ?? 'calendar'}` },
        ],
      }],
    });
    ...
    const breaks = JSON.parse(text);
    if (!Array.isArray(breaks)) throw new SyntaxError('Expected an array');
